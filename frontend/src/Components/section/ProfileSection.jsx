...
const ProfileSection = ({ id,  name, username, profilePic, bio, position, postsCount, followerCount, followingCount}) => {
    ...
    useEffect(() => {
      if (message === "User unfollowed successfully") {
        setFollowing((prevFollowing) => prevFollowing.filter(val => val !== id));
    ...
    <>
        ...
        <div className="flex flex-col gap-5 sm:flex-row">
            ...
            <div className="text-center sm:text-left">
                ...
                <p className="text-gray-700">{position ?? "Update Your Job/Position At Edit Profile"}</p>
            </div>
        </div>
        {user && user.id === id && (
        ...
        {bio === null ? "Please Update Your Bio At Update Profile" : bio}
        </p>

        <div className="flex justify-center sm:justify-start space-x-4 mt-4">
          <div>
              <span className="font-bold">{postsCount}</span> <span className="text-gray-500">Posts</span>
          </div>
          <div>
